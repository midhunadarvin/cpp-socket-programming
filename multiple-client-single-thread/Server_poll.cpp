 ...
 *
 * Compile :
 * g++ -o Server.exe Server_poll.cpp // compile and link
 *
 * Execute :
 * ./Server.exe 8000
 */
...
#include <poll.h>     // poll function and related definitions
...
int main(int argc, char *argv[]) {
  ...
  // initial pollfd array
  struct pollfd fds[MAX_CONNECTIONS + 1];
  fds[0].fd = sock_file_descriptor;
  fds[0].events = POLLIN;
  ...
  while (true) {
    int ready = poll(fds, MAX_CONNECTIONS + 1, 1000);
    if (ready > 0) {
      if (fds[0].revents & POLLIN) {
        ...
        for (i = 1; i <= MAX_CONNECTIONS; i++) {
          if (fds[i].fd == -1) {
            printf(
                "Adding to list of sockets at index: %d, file_descriptor: %d\n",
                i, client_socket_file_descriptor);
            fds[i].fd = client_socket_file_descriptor;
