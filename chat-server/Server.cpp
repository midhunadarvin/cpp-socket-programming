...
int num_of_clients = sizeof(client_socket_fd_map) / sizeof(client_socket_fd_map[0]);

// We create a map that lets us know that clients are connected
std::map<int, int> client_to_client_map;
...
void processRequest(int client_socket_file_descriptor)
{
    ...
    // Find out the receiving client id ( recepient client )
    int recepient_file_descriptor = -1;
    if (client_to_client_map.find(client_socket_file_descriptor) != client_to_client_map.end()) { 
        // Found the recepient
        recepient_file_descriptor = client_to_client_map[client_socket_file_descriptor];
    }
    
    if (recepient_file_descriptor > 0) {
        printf("Recipent client file descriptor : %d\n", recepient_file_descriptor);
        client_messages_queue.insert(std::pair<int, std::string>(recepient_file_descriptor, std::string(buffer)));
    }
...
void processRequest()
{
    ...
    if (FD_ISSET(sock_file_descriptor, &readfds))
    {
        ...
        for (i = 0; i < num_of_clients; i++)
        {
            if (client_socket_fd_map[i] == 0)
            {
                ...
                // For the chat server, we need to keep track of incoming and out going connections
                ...
                {
                    if (client_socket_fd_map[j] != 0 && (j != i))
                    {
                        printf("Found first client connection at index %d, file_descriptor : %d\n", j, client_socket_fd_map[j]);
                        client_to_client_map.insert(std::pair<int,int>(client_socket_file_descriptor, client_socket_fd_map[j]));
                        client_to_client_map.insert(std::pair<int,int>(client_socket_fd_map[j], client_socket_file_descriptor));
                        break;
                    }
                }
    ...
    // we need to check the existing client connections whether we have any new requests
    for (int i = 0; i < num_of_clients; i++)
    {
        if (client_socket_fd_map[i] != 0)
        {
            printf("Found existing client connection at index %d, file_descriptor : %d\n", i, client_socket_fd_map[i]);
            ...
            if (FD_ISSET(client_socket_fd_map[i], &writefds)) {
                printf("There are messages in queue that are to be written to the file_descriptor: %d\n", client_socket_fd_map[i]);
                char *message = (char *) (client_messages_queue[client_socket_fd_map[i]]).c_str();
                printf("Message : %s", message);
                sendRequest(client_socket_fd_map[i], message);
                client_messages_queue.erase(client_socket_fd_map[i]);
            }
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    if (!setsockopt(sock_file_descriptor, SOL_SOCKET, SO_REUSEADDR, (const char *)&optValue, (socklen_t)optLength))
    {
    ...
    while (true)
    {
        ...
        // If we have connected to any clients, we need to make sure to add their file descriptors also to the fd sets
        for (int i = 0; i < num_of_clients; i++)
        {
            if (client_socket_fd_map[i] != 0)
            {
                // increment the max file descriptor
                if (client_socket_fd_map[i] > max_file_descriptor) max_file_descriptor = client_socket_fd_map[i];
                ...
                if (client_messages_queue.find(client_socket_fd_map[i]) != client_messages_queue.end()) {
                    // add to the write FD_SET
                    FD_SET(client_socket_fd_map[i], &writefds);
                }
            }
        }
    ...
    for (int i = 0; i < max_file_descriptor + 1; i++)
    {
        if (client_socket_fd_map[i] != 0)
        {
